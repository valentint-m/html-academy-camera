  ...
  return (
    <li className="basket-item">
      ...
      <div className="quantity">
        <button className="btn-icon btn-icon--prev" aria-label="уменьшить количество товара" disabled={cameraInCart.number === CameraInCartCountBoundaryValue.Min || isSubmitting} onClick={handleDecreaseItemCountButtonClick} >
        ...
        <button className="btn-icon btn-icon--next" aria-label="увеличить количество товара" disabled={cameraInCart.number === CameraInCartCountBoundaryValue.Max || isSubmitting} onClick={handleIncreaseItemCountButtonClick} >
          <svg width="7" height="12" aria-hidden="true">
            <use xlinkHref="#icon-arrow"></use>
          </svg>
        </button>
      ...
      <button className="cross-btn" type="button" aria-label="Удалить товар" onClick={() => onRemoveItemButtonClick(cameraInCart.camera)} disabled={isSubmitting} >
